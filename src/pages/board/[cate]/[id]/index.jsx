...
import ModalDelete from '../../../../components/Board/Modals/Modal_delete';
...
const Board = (props) => {
  return useObserver(() => {
    ...
    const setIsWriter = () => {
      if (state.writer === state.user) state.isWriter = true;
    ...
    const onClickDislike = async () => {
      if (state.login) {
        ...
        state.events.dislike = state.events.dislike === 0
          ? state.events.dislike + 1
          : state.events.dislike - 1;
      }
    };
    ...
    const handleOkDeleteModal = () => {
      const boardDeleteRes = async () => {
        return await BoardAPI.delete({ id: boardId });
      };
      boardDeleteRes();
      router.push('/board/[cate]', `/board/${boardCate}`);
      state.modal.delete = false;
      ...
      <>
        <div className={props.className}>
          ...
          <div className="header_bottom">
            <span>
              <strong>작성일:</strong>
              {formatDate(state.data.createdDate)}
            </span>
            <p>
              <strong>작성자: </strong>
              <Tooltip title={checkAdminWriter()}>
          ...
          <div className="main_container">
            ...
            <Row justify="space-between" className="main_container_top" style={state.isWriter ? { paddingBottom: 0 } : { paddingBottom: '10px' }}>
              {/* 해당 게시글 조회수 & 댓글수 & 좋아요수 */}
              <Row>
                <span className="main_container_top_left ">
                  <EyeOutlined />
                  {numFormatter(state.data.viewCount)}
                </span>
                <Tooltip title={state.login ? '좋아요' : '로그인 해주세요'}>
                  <span className="main_container_top_left event" onClick={onClickLike} >
                    <LikeOutlined />
                    {numFormatter(state.events.like)}
                  </span>
                </Tooltip>
                <Tooltip title={state.login ? '싫어요' : '로그인 해주세요'}>
                  <span className="main_container_top_left event" onClick={onClickDislike}>
                    <DislikeOutlined />
                    {numFormatter(state.events.dislike)}
                  </span>
                </Tooltip>
              </Row>
          ...
          <div className="comment-section">
            ...
            <Comments
              queryId={boardId}
              data={props.comments.body}
              isAdmin={global.state.user.role}
            />
          </div>
