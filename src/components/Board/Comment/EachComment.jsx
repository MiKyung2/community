...
const EachComment = (props) => {
  return useObserver(() => {
    ...
    const handleOkDelete = () => {
      const deleteComment = async () => { return CommentAPI.delete({ id: data.id }); };
      deleteComment();
    ...
    const writer = () => {
      ...
      return (
        <Tooltip title={title}>
          <span className="hover" onClick={moveToWriterProfile}>
            {state.comment.writer}
          </span>
        </Tooltip>
      );
    };
    ...
    return (
      <div>
        <Comment
          actions={checkAdmin()}
          author={writer()}
          avatar={<Avatar src={state.comment.img} alt="profile img" />}
          content={<p>{state.comment.contents}</p>}
          datetime={(
